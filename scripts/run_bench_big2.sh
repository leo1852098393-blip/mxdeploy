...
echo "===== [1/4] 启动 14B-INT4 (enforce-eager, util=0.8, max_len=4096) ====="
...
curl -s http://127.0.0.1:8000/health || { echo "14B 启动失败"; grep -E 'Error|error' /tmp/vllm_14b.log | tail -5; exit 1; }
...
curl -s http://127.0.0.1:8000/health || { echo "GLM 启动失败"; grep -E 'Error|error' /tmp/vllm_glm.log | tail -5; exit 1; }
