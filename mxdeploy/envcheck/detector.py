...
# 沐曦适配版依赖标识（版本号中应包含 +metax 或 +maca）
...
def check_mx_smi() -> tuple[str, int, int, str]:
    ...
    if shutil.which("mx-smi") is None:
        return "未知", 0, 0, "mx-smi 未找到（非沐曦 GPU 环境或驱动未安装）"
...
def check_torch() -> CheckResult:
    try:
        ...
        if any(m in ver for m in METAX_MARKERS):
            return CheckResult("torch", STATUS_OK, detail, "沐曦适配版")
        # torch 装了但没 metax 标记 → 可能是公版覆盖了官方适配
        if torch.cuda.is_available() and "meta" in str(torch.cuda.get_device_name(0)).lower():
            return CheckResult("torch", STATUS_WARN, detail, "检测到沐曦设备但版本无 +metax 标记，请确认是否为官方适配版")
        return CheckResult(
            "torch", STATUS_WARN, detail,
            "未检测到 +metax/+maca 标记，若在沐曦环境请使用官方适配版（pip install torch 会覆盖！）",
        )
    except ImportError:
        return CheckResult(
            "torch", STATUS_WARN, "torch 未安装",
            "部署需要沐曦适配版 PyTorch（版本含 +metax）",
        )
...
def run_all() -> EnvReport:
    ...
    if count == 0:
        report.checks.append(CheckResult("mx-smi", STATUS_ERROR, mx_detail, "未检测到沐曦 GPU，检查驱动或确认运行在模力方舟实例上"))
    else:
