...
def run(
    ...
    gpu_memory_utilization: float = typer.Option(0.8, "--gpu-memory-utilization", "-g", help="GPU 显存利用率 (0~1)，默认 0.8（0.9 在小显存上易 OOM）"),
    ...
    enforce_eager: bool = typer.Option(False, "--enforce-eager", help="跳过 torch.compile（大模型/显存不足时使用，如 14B 量化）"),
    trust_remote_code: bool = typer.Option(False, "--trust-remote-code", help="允许加载模型仓库自定义代码（GLM 系模型需要）"),
    quantization: str = typer.Option(None, "--quantization", help="量化方式: gptq / awq（默认按模型名自动检测）"),
    extra_args: str = typer.Option(None, "--extra-args", help="额外 vLLM 启动参数（空格分隔，如 '--max-num-seqs 4 --swap-space 8'）"),
) -> None:
